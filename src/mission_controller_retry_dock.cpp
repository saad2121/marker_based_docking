    ...
    : nh_(), pnh_("~"), state_(State::START), mission_index_(0)
{
    ...
    dock_target_sub_ = nh_.subscribe(
        "/dock_target", 1,
        &MissionController::dockTargetCB, this);
...
private:
  ...
  /* -------- ROS Interfaces -------- */
    ...
    ros::Subscriber dock_target_sub_;
  ...
  /* -------- FSM -------- */
    ...
    int  dock_target_lost_count_ = 0;
    int dock_target_lost_count_limit_ = 5;
...
void dockTargetCB(const docking_marker::DockTarget::ConstPtr& msg)
{
    if (state_ != State::WAIT_DOCK || !docking_active_)
    return;

    if (!msg->valid)  // target lost
    {
    dock_target_lost_count_++;

    ROS_WARN_THROTTLE(1.0,
        "Dock target lost (%d/10)", dock_target_lost_count_);

    if (dock_target_lost_count_ > dock_target_lost_count_limit_)
        {
        ROS_ERROR("Dock target lost %d times -> Restart docking", dock_target_lost_count_limit_);
